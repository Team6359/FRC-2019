...
public class RobotController {
	...
	public void update(boolean a, boolean b, int DPad, boolean rB, boolean lB, boolean back, boolean y, boolean start,
			boolean start2) {
		// Select Ball/Hatch
		if (a) {
			object = 0;
		...
		if (object == 0) {
			...
			case 180:
				...
				break;
			case 270:
				Robot.mathHandler.setCurrentPosition(RobotMap.setPointHatchFloor[0], RobotMap.setPointHatchFloor[1],
						RobotMap.setPointHatchFloor[2]);
				break;
			}
		} else if (object == 1) {
			switch (DPad) {
			...
			case 180:
				...
				break;
			case 270:
				Robot.mathHandler.setCurrentPosition(RobotMap.setPointCargoFloor[0], RobotMap.setPointCargoFloor[1],
						RobotMap.setPointCargoFloor[2]);
				break;
			}
		}
		...
		if (object == 1) {
			...
			if (rB) {
				...
				// if (Robot.mathHandler.getCurrentPosition()[2] ==
				// RobotMap.setPointHatchFloor[2]){
				// Robot.mathHandler.setCurrentPosition(Robot.mathHandler.getCurrentPosition()[0]
				// - 0.5, Robot.mathHandler.getCurrentPosition()[1],
		...
		if (back) {
			if (object == 0) {
				...
				Robot.mathHandler.setCurrentPosition(RobotMap.setPointCargoIntake[0], RobotMap.setPointCargoIntake[1],
						RobotMap.setPointCargoIntake[2]);
			}
		}
		...
		if (start) {
			...
			Robot.mathHandler.setCurrentPosition(Robot.mathHandler.getCurrentPosition()[0],
					Robot.mathHandler.getCurrentPosition()[1], 0);
		}

		if (System.currentTimeMillis() - startTimeCarry > 500 && System.currentTimeMillis() - startTimeCarry < 1000) {
			if (object == 1) {
				Robot.mathHandler.setCurrentPosition(RobotMap.setPointCargoTravel[0], RobotMap.setPointCargoTravel[1],
						RobotMap.setPointCargoTravel[2]);
			} else if (object == 0) {
				Robot.mathHandler.setCurrentPosition(RobotMap.setPointHatchTravel[0], RobotMap.setPointHatchTravel[1],
						RobotMap.setPointHatchTravel[2]);
			}
			System.out.println("FOLD");
		}

		if (start2) {
			Robot.mathHandler.setCurrentPosition(RobotMap.setPointTest[0], RobotMap.setPointTest[1],
					RobotMap.setPointTest[2]);
		}
	...
	// RobotMap.setPointCargoFloor[1], 0);
